...
function Category() {
    ...
    const [focusedCategory, setFocusedCategory] = useState(param.id);
        ...
        <div className="container ">
            <div className="flex">
                <section className='text-gray-600 body-font w-2/12 cursor-pointer hidden lg:block '>
                    {/* Category Section */}
                    <section className="text-gray-600 body-font w-2/12 cursor-pointer hidden lg:block">
                        <div className="fixed w-2/12 h-full top-0 overflow-y-scroll">
                            <div className="px-5 pt-20 pb-4 mx-auto">
                                <h2 className="text-lg font-bold text-black">Price Filter</h2>
                                <div className="px-1 mx-auto">
                                    <div onClick={() => handleClick(0, 2000)} onFocus={() => handleFocus(0, 2000)} onBlur={handleBlur} tabIndex="0" className={`flex justify-center items-center text-black group font-bold hover:shadow-xl px-2 py-1 duration-300 ${ focusedRange === '0-2000' || selectedRange === '0-2000' ? 'bg-purple-200 text-purple-800' : '' }`} >
                                        <h3 className="duration-200 text-xs md:text-sm lg:text-base group-hover:text-purple-800">
                                            ALL Products
                                        </h3>
                                    </div>
                                    {price.map((ele, ind) => {
                                        ...
                                        return (
                                            <div key={ind} onClick={() => handleClick(ele.min, ele.max)} onFocus={() => handleFocus(ele.min, ele.max)} onBlur={handleBlur} tabIndex="0" className={`flex items-center rounded my-[2px] text-gray-500 hover:text-black group font-bold hover:shadow-xl px-2 py-1 group duration-300 ${ focusedRange === rangeKey || selectedRange === rangeKey ? 'bg-purple-200 text-purple-800' : '' }`} >
                                    ...
                                    {category &&
                                        category.map((ele, ind) => (
                                            <li key={ind} className={`text-gray-500  hover:text-black group font-bold hover:shadow-xl px-2 py-1 group duration-300 ${ focusedCategory === ele ? 'bg-purple-200 text-purple-800 shadow-lg' : '' }`} >
                                                <Link to={`/category/${ele}`} className='block' onClick={() => { handleClick(0, 2000); setSelectedRange(null); }} onFocus={() => handleFocusCategory(ele)} onBlur={handleBlurCategory} tabIndex="0" > {ele} </Link>
                                            </li>
                                        ))}
                                </ul>
                            </div>
